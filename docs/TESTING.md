...
| 6  | MachineDeployment self-heal (delete worker)           | ✅      | v0.2.2          | CAPI MD recreates Machine after explicit delete. Re-tested v0.2.2: drain blocks ~3 min on tigera-operator (single-replica, no PDB) until a 2nd worker becomes available. No actual MachineHealthCheck CR ships in the templates today (gap for v0.3.0) |
